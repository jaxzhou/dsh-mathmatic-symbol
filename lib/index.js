...
var MAX_TICKS = 40;
function fail2(path5, message) {
  throw new Error(`figure spec: ${path5} ${message}`);
}
function asRecord(value, path5) {
  if (typeof value !== "object" || value === null || Array.isArray(value)) fail2(path5, "must be an object");
  return value;
}
function assertKeys(raw, allowed, path5) {
  const unknown = Object.keys(raw).filter((key) => !allowed.includes(key));
  if (unknown.length > 0) {
    fail2(path5, `has unknown key(s) ${unknown.map((key) => `"${key}"`).join(", ")}; allowed: ${allowed.join(", ")}`);
  }
}
function optionalNumber(raw, key, path5, min, max) {
  const value = raw[key];
  if (value === void 0) return void 0;
  if (typeof value !== "number" || !Number.isFinite(value)) fail2(`${path5}.${key}`, "must be a finite number");
  if (min !== void 0 && value < min) fail2(`${path5}.${key}`, `must be at least ${min}`);
  if (max !== void 0 && value > max) fail2(`${path5}.${key}`, `must be at most ${max}`);
  return value;
}
function numberField(raw, key, path5, fallback) {
  return optionalNumber(raw, key, path5) ?? fallback;
}
function optionalBoolean(raw, key, path5) {
  const value = raw[key];
  if (value === void 0) return void 0;
  if (typeof value !== "boolean") fail2(`${path5}.${key}`, "must be a boolean");
  return value;
}
function optionalString(raw, key, path5) {
  const value = raw[key];
  if (value === void 0) return void 0;
  if (typeof value !== "string" || value.length === 0) fail2(`${path5}.${key}`, "must be a non-empty string");
  return value;
}
function readColor(raw, key, path5, fallback) {
  const value = optionalString(raw, key, path5);
  if (value === void 0) return fallback;
  if (!isSafeColor(value)) fail2(`${path5}.${key}`, `"${value}" is not a flat CSS color`);
  return value;
}
function readStyle(raw, path5) {
  const value = optionalString(raw, "style", path5) ?? "solid";
  if (value !== "solid" && value !== "dashed" && value !== "dotted") {
    fail2(`${path5}.style`, 'must be "solid", "dashed", or "dotted"');
  }
  return value;
}
function readArrow(raw, path5, fallback) {
  const value = optionalString(raw, "arrow", path5) ?? fallback;
  if (value !== "none" && value !== "end" && value !== "both" && value !== "start") {
    fail2(`${path5}.arrow`, 'must be "none", "end", "start", or "both"');
  }
  return value;
}
function readRange(raw, key, path5) {
  const value = raw[key];
  if (value === void 0) return void 0;
  if (!Array.isArray(value) || value.length !== 2) fail2(`${path5}.${key}`, "must be a [min, max] pair");
  const first = value[0];
  const second = value[1];
  if (typeof first !== "number" || typeof second !== "number" || !Number.isFinite(first) || !Number.isFinite(second)) {
    fail2(`${path5}.${key}`, "must be a [min, max] pair of finite numbers");
  }
  if (first >= second) fail2(`${path5}.${key}`, "must satisfy min < max");
  return [first, second];
}
function readOffset(raw, path5, fallback) {
  const value = raw.label_offset;
  if (value === void 0) return fallback;
  if (!Array.isArray(value) || value.length !== 2) {
    fail2(`${path5}.label_offset`, "must be a [dx, dy] pair");
  }
  const first = value[0];
  const second = value[1];
  if (typeof first !== "number" || typeof second !== "number" || !Number.isFinite(first) || !Number.isFinite(second)) {
    fail2(`${path5}.label_offset`, "must be a [dx, dy] pair of finite numbers in screen pixels (y grows downward)");
  }
  return [first, second];
}
function readAnchor(raw, path5, fallback) {
  const value = optionalString(raw, "anchor", path5) ?? fallback;
  if (value !== "start" && value !== "middle" && value !== "end") fail2(`${path5}.anchor`, 'must be "start", "middle", or "end"');
  return value;
}
function readValign(raw, path5, fallback) {
  const value = optionalString(raw, "valign", path5) ?? fallback;
  if (value !== "baseline" && value !== "middle" && value !== "top" && value !== "bottom") {
    fail2(`${path5}.valign`, 'must be "baseline", "middle", "top", or "bottom"');
  }
  return value;
}
function readFill(raw, path5) {
  const fill = optionalString(raw, "fill", path5);
  if (fill === void 0 || fill === "none") return { fill: null, fillOpacity: 0 };
  if (!isSafeColor(fill)) fail2(`${path5}.fill`, `"${fill}" is not a flat CSS color`);
  return { fill, fillOpacity: optionalNumber(raw, "fill_opacity", path5, 0, 1) ?? 0.18 };
}
...
function scalar(raw, path5, vars) {
  if (typeof raw === "number") {
    if (!Number.isFinite(raw)) fail2(path5, "must be a finite number");
    return raw;
  }
  if (typeof raw === "string") {
    try {
      const value = evaluateExpression(raw, vars);
      if (!Number.isFinite(value)) fail2(path5, `"${raw}" evaluated to ${String(value)}, which is not a usable coordinate`);
      return value;
    } catch (error) {
      fail2(path5, `"${raw}" is not a usable expression: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  fail2(path5, "must be a number or an expression string");
}
function pointRef(raw, path5, state) {
  if (typeof raw === "string") {
    const found = state.points.get(raw);
    if (found === void 0) {
      const known = [...state.points.keys()];
      fail2(path5, `references unknown point "${raw}"${known.length > 0 ? `; defined so far: ${known.join(", ")}` : '; define it with a "point" element first'}`);
    }
    return [found[0], found[1]];
  }
  if (Array.isArray(raw) && raw.length === 2) {
    return [scalar(raw[0], `${path5}[0]`, state.vars), scalar(raw[1], `${path5}[1]`, state.vars)];
  }
  fail2(path5, "must be a point label string or an [x, y] pair");
}
function pointList(raw, path5, state) {
  if (!Array.isArray(raw) || raw.length === 0) fail2(path5, "must be a non-empty array of points");
  return raw.map((item, index) => pointRef(item, `${path5}[${index}]`, state));
}
...
function pushPolyline(state, pts, path5, element, options) {
  state.prims.push({
    kind: "polyline",
    pts,
    closed: options.closed,
    color: readColor(element, "color", path5, DEFAULT_INK),
    width: numberField(element, "width", path5, DEFAULT_STROKE),
    style: readStyle(element, path5),
    arrow: options.arrow,
    fill: options.fill,
...
function centroid(points2) {
  ...
  return [x / points2.length, y / points2.length];
}
function unitVector(x, y, path5) {
  const length = Math.hypot(x, y);
  if (!Number.isFinite(length) || length < 1e-9) fail2(path5, "coincides with the vertex, so no angle can be drawn");
...
function resolveElement(raw, index, state) {
  const path5 = `elements[${index}]`;
  const element = asRecord(raw, path5);
  const type = element.type;
  if (typeof type !== "string") fail2(`${path5}.type`, "must be a string naming the element type");
  const allowed = ELEMENT_KEYS[type];
  if (allowed === void 0) {
    fail2(`${path5}.type`, `"${type}" is not a known element type; known types: ${Object.keys(ELEMENT_KEYS).join(", ")}`);
  }
  assertKeys(element, allowed, path5);
  switch (type) {
    case "point": {
      const at = pointRef(element.at, `${path5}.at`, state);
      const label = optionalString(element, "label", path5);
      ...
      state.prims.push({
        ...
        y: at[1],
        color: readColor(element, "color", path5, DEFAULT_INK),
        size: numberField(element, "size", path5, 3),
        filled: !(optionalBoolean(element, "open", path5) ?? false)
      });
      if (label !== void 0) {
        const [dx, dy] = readOffset(element, path5, [10, -10]);
        pushLabel(state, {
          x: at[0],
          y: at[1],
          tex: label,
          size: numberField(element, "label_size", path5, LABEL_SIZE),
          color: LABEL_COLOR,
          dx,
          dy
        });
    ...
    case "segment":
    case "vector": {
      const from = pointRef(element.from, `${path5}.from`, state);
      const to = pointRef(element.to, `${path5}.to`, state);
      pushPolyline(state, [from, to], path5, element, {
        closed: false,
        arrow: readArrow(element, path5, type === "vector" ? "end" : "none"),
        fill: null,
        fillOpacity: 0
      });
      const label = optionalString(element, "label", path5);
      if (label !== void 0) {
        const [dx, dy] = readOffset(element, path5, [0, -10]);
        const [x, y] = midpoint(from, to);
        pushLabel(state, { x, y, tex: label, size: numberField(element, "label_size", path5, LABEL_SIZE), color: LABEL_COLOR, dx, dy });
      }
      return;
    }
    case "polyline":
    case "polygon": {
      const points2 = pointList(element.points, `${path5}.points`, state);
      const closed = optionalBoolean(element, "closed", path5) ?? type === "polygon";
      const { fill, fillOpacity } = readFill(element, path5);
      pushPolyline(state, points2, path5, element, {
        closed,
        arrow: readArrow(element, path5, "none"),
        fill,
        fillOpacity
      });
      const label = optionalString(element, "label", path5);
      if (label !== void 0) {
        const [x, y] = centroid(points2);
        pushLabel(state, { x, y, tex: label, size: numberField(element, "label_size", path5, LABEL_SIZE), color: LABEL_COLOR, dx: 0, dy: 0 });
      }
      return;
    }
    case "line": {
      const through = pointList(element.through, `${path5}.through`, state);
      if (through.length !== 2) fail2(`${path5}.through`, "must contain exactly two points");
      const a = through[0];
      const b = through[1];
      const extendMode = optionalString(element, "extend", path5) ?? "both";
      if (extendMode !== "both" && extendMode !== "forward" && extendMode !== "backward" && extendMode !== "none") {
        fail2(`${path5}.extend`, 'must be "both", "forward", "backward", or "none"');
      }
      state.lines.push({
        a,
        b,
        ...
        style: readStyle(element, path5),
        arrow: readArrow(element, path5, "none"),
        label: optionalString(element, "label", path5) ?? null,
        labelSize: numberField(element, "label_size", path5, LABEL_SIZE),
        labelOffset: readOffset(element, path5, [0, -10])
      });
    ...
    case "ray": {
      const from = pointRef(element.from, `${path5}.from`, state);
      const through = pointRef(element.through, `${path5}.through`, state);
      state.lines.push({
        a: from,
        b: through,
        mode: "forward",
        ...
        width: numberField(element, "width", path5, DEFAULT_STROKE),
        style: readStyle(element, path5),
        arrow: readArrow(element, path5, "end"),
        label: optionalString(element, "label", path5) ?? null,
        labelSize: numberField(element, "label_size", path5, LABEL_SIZE),
        labelOffset: readOffset(element, path5, [0, -10])
      });
    ...
    case "circle": {
      const center = pointRef(element.center, `${path5}.center`, state);
      const through = element.through === void 0 ? void 0 : pointRef(element.through, `${path5}.through`, state);
      const declared = optionalNumber(element, "radius", path5, 0);
      const radius = declared ?? (through !== void 0 ? Math.hypot(through[0] - center[0], through[1] - center[1]) : void 0);
      if (radius === void 0 || radius <= 0) fail2(path5, 'needs a positive "radius" or a "through" point');
      const { fill, fillOpacity } = readFill(element, path5);
      state.prims.push({
        kind: "ellipse",
        cx: center[0],
    ...
    case "arc": {
      const center = pointRef(element.center, `${path5}.center`, state);
      const radius = optionalNumber(element, "radius", path5, 0);
      if (radius === void 0 || radius <= 0) fail2(path5, 'needs a positive "radius"');
      const start = optionalNumber(element, "start", path5);
      const end = optionalNumber(element, "end", path5);
      if (start === void 0 || end === void 0) fail2(path5, 'needs "start" and "end" angles in degrees');
    ...
    case "angle": {
      ...
      state.prims.push({
        ...
        vx: at[0],
        vy: at[1],
        u1: unitVector(from[0] - at[0], from[1] - at[1], `${path5}.from`),
        u2: unitVector(to[0] - at[0], to[1] - at[1], `${path5}.to`),
        radiusPx: numberField(element, "radius", path5, ANGLE_RADIUS),
        right: optionalBoolean(element, "right", path5) ?? false,
        color: readColor(element, "color", path5, DEFAULT_INK),
        width: numberField(element, "width", path5, DEFAULT_STROKE),
        label: optionalString(element, "label", path5) ?? null,
        labelSize: numberField(element, "label_size", path5, LABEL_SIZE)
    ...
    case "curve": {
      const expression = optionalString(element, "y", path5);
      if (expression === void 0) fail2(path5, 'needs "y" (an expression in x)');
      const domain = readRange(element, "domain", path5) ?? [-5, 5];
      const samples = Math.round(numberField(element, "samples", path5, 400));
      if (samples < 2 || samples > MAX_SAMPLES) fail2(`${path5}.samples`, `must be between 2 and ${MAX_SAMPLES}`);
      state.hasPlot = true;
      ...
      if (pts.length < 2) {
        fail2(`${path5}.y`, `"${expression}" produced no drawable points on ${JSON.stringify(domain)}${firstError !== void 0 ? ` (${firstError})` : ""}`);
      }
      if (failures > 0) state.warnings.push(`${path5}: ${failures} sample(s) were outside the function's domain and were skipped`);
      pushPolyline(state, pts, path5, element, { closed: false, arrow: "none", fill: null, fillOpacity: 0 });
      return;
    }
    case "parametric": {
      const xExpression = optionalString(element, "x", path5);
      const yExpression = optionalString(element, "y", path5);
      if (xExpression === void 0 || yExpression === void 0) fail2(path5, 'needs "x" and "y" expressions in t');
      const range = readRange(element, "range", path5) ?? [0, Math.PI * 2];
      const samples = Math.round(numberField(element, "samples", path5, 400));
      if (samples < 2 || samples > MAX_SAMPLES) fail2(`${path5}.samples`, `must be between 2 and ${MAX_SAMPLES}`);
      state.hasPlot = true;
      ...
      if (pts.length < 2) fail2(path5, `produced no drawable points${firstError !== void 0 ? ` (${firstError})` : ""}`);
      if (failures > 0) state.warnings.push(`${path5}: ${failures} sample(s) were skipped`);
      pushPolyline(state, pts, path5, element, { closed: false, arrow: "none", fill: null, fillOpacity: 0 });
      return;
    }
    case "polar": {
      const rExpression = optionalString(element, "r", path5);
      if (rExpression === void 0) fail2(path5, 'needs "r" (an expression in theta)');
      const range = readRange(element, "range", path5) ?? [0, Math.PI * 2];
      const samples = Math.round(numberField(element, "samples", path5, 400));
      if (samples < 2 || samples > MAX_SAMPLES) fail2(`${path5}.samples`, `must be between 2 and ${MAX_SAMPLES}`);
      state.hasPlot = true;
    ...
    case "text": {
      const at = pointRef(element.at, `${path5}.at`, state);
      const text = optionalString(element, "text", path5);
      if (text === void 0) fail2(path5, 'needs "text"');
      pushLabel(state, {
        x: at[0],
        y: at[1],
        tex: text,
        size: numberField(element, "size", path5, LABEL_SIZE),
        color: readColor(element, "color", path5, LABEL_COLOR),
        dx: 0,
        dy: 0,
        anchor: readAnchor(element, path5, "middle"),
        valign: readValign(element, path5, "middle"),
        rotate: numberField(element, "rotate", path5, 0)
      });
      extend(state, at[0], at[1]);
      return;
    }
    default:
      fail2(`${path5}.type`, `"${type}" is declared but not implemented`);
  }
}
...
function pathData(points2, closed) {
  const parts = [];
  for (const [index, point] of points2.entries()) {
    parts.push(`${index === 0 ? "M" : "L"}${formatNumber(point[0])} ${formatNumber(point[1])}`);
  }
  if (closed) parts.push("Z");
  return parts.join(" ");
}
function visibleRuns(points2, width, height, margin) {
  const runs = [];
  let current = [];
  for (const point of points2) {
...
async function emitPrim(prim, transform) {
  switch (prim.kind) {
    ...
    case "arc": {
      ...
      const path5 = `<path d="M${formatNumber(start[0])} ${formatNumber(start[1])} A${formatNumber(prim.r * transform.sx)} ${formatNumber(prim.r * transform.sy)} 0 ${Math.abs(delta) > Math.PI ? 1 : 0} ${delta >= 0 ? 0 : 1} ${formatNumber(end[0])} ${formatNumber(end[1])}"${strokeAttributes(prim.color, prim.width, prim.style)}/>`;
      if (prim.arrow === "none") return path5;
      ...
      return path5 + arrowMarkup(tail, end, prim.color);
    }
    case "dot": {
...
async function renderFormulaSvg(latex, options) {
  ...
  return { svg, width, height, depthPx: rendered.depth / UNITS_PER_EM2 * options.fontSize, warnings };
}

// src/prompt.ts
var TOOL_NAMES = ["math_formula", "math_figure", "math_convert", "math_document"];
var PROMPT_SECTION_NAME = "tool:math-symbol";
function mathToolGuidance(available) {
  ...
  if (available.includes("math_figure")) {
    bullets.push(
      "- `math_figure` \u2014 geometry constructions and function/parametric/polar plots from a JSON spec (points, segments, lines, polygons, circles, arcs, angles, curves). Declare the elements; do not draw by hand."
    );
  }
  ...
  if (available.includes("math_document")) {
    bullets.push(
      "- `math_document` \u2014 assemble a Markdown/HTML/LaTeX document whose formulas and figures are already rendered and inserted. Write the body with `$inline$`, `$$display$$`, and `[[figure:name]]` tokens; pass `figures` as a name \u2192 spec map. This replaces writing image paths or embedding markup by hand."
    );
  }
  return "Math and figures: when a formula or a geometric/function figure is needed as an *image*, or a document with such images embedded is the deliverable (or the user asks for one), use these tools rather than writing your own rendering code:\n" + bullets.join("\n") + "\nEvery call writes the image files into the session workspace, names them content-addressed, and returns paths plus ready-to-paste Markdown/HTML/LaTeX snippets \u2014 use what the tool returns; do not re-derive paths, re-encode images, or re-implement the rendering with SVG, LaTeX tooling, or a plotting library. Ordinary chat answers keep using LaTeX text directly; these tools are for image or document deliverables.";
}
function registerMathPromptSection(ctx) {
  ...
  systemPrompt.section({
    ...
    text: ({ scope }) => {
      const available = TOOL_NAMES.filter((name2) => ctx.tools.get?.(name2, scope) !== void 0);
...
var PREFERRED_VARIANT = {
  markdown: "svg",
  html: "svg",
  latex: "png"
  // \includegraphics cannot read SVG without extra packages
};
function allowedVariants(format) {
  return format === "latex" ? ["png"] : ["svg", "png"];
}
function preferredVariant(format, produced) {
  const preferred = PREFERRED_VARIANT[format];
  if (produced.includes(preferred)) return preferred;
  const fallback = produced[0];
  if (fallback === void 0) throw new Error("no image variant was produced");
  return fallback;
}
var FIGURE_TOKEN_FORMS = [
  { open: "[[figure:", close: "]]" },
  { open: "{{figure:", close: "}}" }
];
function readFigureToken(body, cursor) {
  ...
  for (const form of FIGURE_TOKEN_FORMS) {
    ...
    if (close < 0) continue;
    const name2 = body.slice(cursor + form.open.length, close).trim();
    if (!/^[A-Za-z0-9_-]+$/.test(name2)) continue;
    return { name: name2, next: close + form.close.length };
  }
  return void 0;
}
...
function centimeters(width) {
  return `${(width / 96 * 2.54).toFixed(2)}cm`;
}
function points(pixels) {
  return `${(pixels * 0.75).toFixed(2)}pt`;
}
function inlineReference(format, placement) {
  if (format === "html") {
    const shift = placement.depthPx > 0.5 ? ` style="vertical-align:${formatNumber(-placement.depthPx)}px"` : "";
    return `<img src="${escapeAttribute(placement.reference)}" width="${Math.round(placement.width)}" height="${Math.round(placement.height)}" alt="${escapeAttribute(placement.alt)}"${shift}>`;
  }
...
function displayReference(format, placement) {
  if (format === "html") {
    return `<p style="text-align:center"><img src="${escapeAttribute(placement.reference)}" width="${Math.round(placement.width)}" height="${Math.round(placement.height)}" alt="${escapeAttribute(placement.alt)}"></p>`;
  }
  if (format === "markdown") return `![${placement.alt.replace(/[[\]\n\r]/g, " ")}](${placement.reference})`;
  return `\\begin{center}
...
function renderText(format, text) {
...
function assembleDocument(input) {
  ...
  const leading = [];
  if (input.title !== void 0 && input.title.length > 0) {
...
` + (needsMathJax ? MATHJAX_BOOTSTRAP : "") + `</head>
<body>
${content.trim()}
</body>
</html>
...
function latexSnippet(image) {
  const centimeters2 = (image.width / 96 * 2.54).toFixed(2);
  return `\\includegraphics[width=${centimeters2}cm]{${image.relativePath}}`;
}
...
var MATH_DOCUMENT_VALUE_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: [
    "kind",
...
function formatMathDocumentValue(value) {
  const lines = [
    `<document format="${value.format}" path="${value.doc_path}" bytes="${value.doc_bytes}" math="${value.math}" self_contained="${value.self_contained}">`
  ];
...
function createTool(spec) {
  ...
  const render = spec.output?.render ?? ((value) => formatMathImageValue(value));
  return {
    name: spec.name,
...
// src/tools/convert.ts
var TOOL = "math_convert";
var OWN_KEYS = ["source", "font_size", "color"];
  ...
  return createTool({
    ...
    async execute(raw, exec) {
      ...
      if (extension === ".png") {
        ...
        const size = readPngSize(data);
        if (size === void 0) throw new Error(`${TOOL}: "${requested}" is not a readable PNG`);
...
function readString3(raw, key) {
  const value = raw[key];
  if (value === void 0) return void 0;
  if (typeof value !== "string") throw new Error(`${TOOL2}: "${key}" must be a string`);
  return value;
}
function readBoolean3(raw, key) {
  const value = raw[key];
  if (value === void 0) return void 0;
  if (typeof value !== "boolean") throw new Error(`${TOOL2}: "${key}" must be a boolean`);
  return value;
}
function readNumber3(raw, key, min, max) {
  const value = raw[key];
  if (value === void 0) return void 0;
  if (typeof value !== "number" || !Number.isFinite(value)) throw new Error(`${TOOL2}: "${key}" must be a finite number`);
  if (value < min || value > max) throw new Error(`${TOOL2}: "${key}" must be between ${min} and ${max}`);
  return value;
}
function readEnum(raw, key, allowed) {
  const value = readString3(raw, key);
  if (value === void 0) return void 0;
  if (!allowed.includes(value)) {
    throw new Error(`${TOOL2}: "${key}" must be one of ${allowed.map((item) => `"${item}"`).join(", ")}`);
  }
  return value;
}
function createDocumentTool(ctx, config) {
  void ctx;
  return createTool({
    name: TOOL2,
    output: { schema: MATH_DOCUMENT_VALUE_SCHEMA, render: formatMathDocumentValue },
    description: 'Assemble a document (Markdown, HTML, or LaTeX) with mathematical formulas and figures already rendered and inserted. Write the body with $inline$ / $$display$$ math and [[figure:name]] placeholders; pass the figures as a name \u2192 math_figure spec map. The tool renders every formula and figure, names and writes the images beside the document, and inserts them in the syntax of the chosen format \u2014 relative paths by default, or base64 data URIs with self_contained for a single self-sufficient file. LaTeX output needs PNG images (image_format "png" or "both"); HTML output with math "native" includes a MathJax bootstrap. Use this instead of hand-writing image paths or embedding markup yourself.',
    parameters: {
      type: "object",
      additionalProperties: false,
      required: ["path", "body"],
    ...
    async execute(raw, exec) {
      assertKnownKeys(raw, PARAMETER_KEYS, TOOL2);
      const requestedPath = readString3(raw, "path");
      if (requestedPath === void 0 || requestedPath.trim().length === 0) {
        throw new Error(`${TOOL2}: "path" must be a non-empty workspace-relative path`);
      }
      const bodyRaw = raw.body;
      if (typeof bodyRaw !== "string" || bodyRaw.trim().length === 0) {
        throw new Error(`${TOOL2}: "body" must be a non-empty string`);
      }
      const format = readEnum(raw, "format", ["markdown", "html", "latex"]) ?? "markdown";
      const math = readEnum(raw, "math", ["native", "image"]) ?? "native";
      const imageFormat = readEnum(raw, "image_format", ["svg", "png", "both"]) ?? "both";
      const selfContained = readBoolean3(raw, "self_contained") ?? false;
      const mathjax = readBoolean3(raw, "mathjax") ?? true;
      const title = readString3(raw, "title");
      const scale = readNumber3(raw, "scale", 0.25, 16) ?? config.scale;
      const fontSize = readNumber3(raw, "font_size", 6, 96) ?? config.fontSize;
      const background = readString3(raw, "background") ?? config.background;
      const color = readString3(raw, "color") ?? config.color;
      if (background !== "transparent" && !isSafeColor(background)) {
        throw new Error(`${TOOL2}: "background" must be "transparent" or a flat CSS color`);
      }
      if (!isSafeColor(color)) throw new Error(`${TOOL2}: "color" must be a flat CSS color such as "#111827"`);
      const extension = DOCUMENT_EXTENSION[format];
      const knownExtensions = [".md", ".markdown", ".html", ".htm", ".tex", ".latex"];
      ...
      if (hasKnownExtension && !lower.endsWith(extension) && !(format === "html" && lower.endsWith(".htm"))) {
        throw new Error(`${TOOL2}: "path" ends in a different document extension than format "${format}" (${extension})`);
      }
      const documentRelative = hasKnownExtension ? requestedPath : `${requestedPath}${extension}`;
      if (format === "latex" && selfContained) {
        throw new Error(`${TOOL2}: self_contained is not available for LaTeX output; \\includegraphics needs a file`);
      }
      const variants = imageFormat === "both" ? ["svg", "png"] : [imageFormat];
      const usable = allowedVariants(format);
      if (!variants.some((variant) => usable.includes(variant))) {
        throw new Error(
          `${TOOL2}: LaTeX documents embed raster images; use image_format "png" or "both" (got "${imageFormat}")`
        );
      }
      const figuresRaw = raw.figures;
      if (figuresRaw !== void 0 && (typeof figuresRaw !== "object" || figuresRaw === null || Array.isArray(figuresRaw))) {
        throw new Error(`${TOOL2}: "figures" must be an object mapping names to figure specs`);
      }
      ...
      for (const name2 of referenced) {
        if (!Object.hasOwn(figures, name2)) {
          const available = Object.keys(figures);
          throw new Error(
            `${TOOL2}: [[figure:${name2}]] has no spec in "figures"` + (available.length > 0 ? `; provided: ${available.join(", ")}` : '; "figures" is empty')
          );
        }
      }
      for (const name2 of Object.keys(figures)) {
        if (!referenced.includes(name2)) warnings.push(`figure "${name2}" was provided but never referenced by [[figure:${name2}]]`);
      }
      ...
      for (const segment of segments) {
        ...
        if (segment.kind === "math") {
          ...
          if (tex.length === 0) throw new Error(`${TOOL2}: a math token is empty after removing delimiters`);
          const key2 = `math\0${segment.display}\0${tex}`;
          let asset2 = renderCache.get(key2);
          if (asset2 === void 0) {
            ...
            if (variants.includes("png")) {
              ...
              if (raster !== void 0) {
                asset2.pngBytes = raster.bytes;
                asset2.pngWidth = raster.width;
                asset2.pngHeight = raster.height;
              }
            }
            renderCache.set(key2, asset2);
          }
          assets.set(segment.index, asset2);
          continue;
        }
        const key = `figure\0${segment.name}`;
        ...
        if (asset === void 0) {
          ...
          const specTitle = typeof spec === "object" && spec !== null && !Array.isArray(spec) ? spec.title : void 0;
          asset = {
      ...
      const documentDirectory = path4.posix.dirname(documentTarget.relativePath);
      const documentBase = path4.posix.basename(documentTarget.relativePath, extension);
      const assetsRelative = readString3(raw, "assets_dir") ?? path4.posix.join(documentDirectory === "." ? "" : documentDirectory, `${documentBase}-assets`);
      let assetsDirectory = null;
      let assetsHostPath = null;
      ...
      for (const [index, asset] of assets) {
        ...
        if (assetsHostPath !== null) {
          if (asset.svgText !== void 0 && variants.includes("svg")) {
            const target = {
              hostPath: path4.join(assetsHostPath, `${asset.baseName}.svg`),
              relativePath: path4.posix.join(assetsDirectory, `${asset.baseName}.svg`),
              root
            };
          ...
          if (asset.pngBytes !== void 0 && variants.includes("png")) {
            const target = {
              hostPath: path4.join(assetsHostPath, `${asset.baseName}.png`),
              relativePath: path4.posix.join(assetsDirectory, `${asset.baseName}.png`),
              root
            };
      ...
      for (const segment of segments) {
        ...
        if (asset === void 0 || writes === void 0) throw new Error(`${TOOL2}: internal error \u2014 missing asset for segment ${segment.index}`);
        const produced = [];
        if (asset.svgText !== void 0) produced.push("svg");
        if (asset.pngBytes !== void 0) produced.push("png");
        const label = segment.kind === "math" ? "a formula" : `figure "${segment.name}"`;
        if (produced.length === 0) throw new Error(`${TOOL2}: no image variant could be produced for ${label}`);
        if (format === "latex" && asset.pngBytes === void 0) {
          throw new Error(`${TOOL2}: LaTeX output needs a PNG for ${label}, but rasterization failed \u2014 see the warnings`);
        }
        const variant = preferredVariant(format, produced);
        const displayWidth = asset.svgWidth ?? (asset.pngWidth !== void 0 ? asset.pngWidth / scale : 0);
        const displayHeight = asset.svgHeight ?? (asset.pngHeight !== void 0 ? asset.pngHeight / scale : 0);
        let reference;
        if (selfContained) {
          const bytes = variant === "svg" ? asset.svgText : asset.pngBytes;
          if (bytes === void 0) throw new Error(`${TOOL2}: no ${variant} data was rendered for self_contained output`);
          ...
          if (relative === void 0) throw new Error(`${TOOL2}: the ${variant} variant was not written`);
          const fromDirectory = path4.posix.dirname(documentTarget.relativePath);
          reference = path4.posix.relative(fromDirectory === "." ? "" : fromDirectory, relative);
        }
...
function createFigureTool(ctx, config) {
  return createTool({
    ...
    async execute(raw, exec) {
      assertKnownKeys(raw, ALL_KEYS2, TOOL3);
...
var TOOL4 = "math_formula";
var OWN_KEYS3 = ["latex", "display", "font_size", "color"];
...
function createFormulaTool(ctx, config) {
  return createTool({
    ...
    async execute(raw, exec) {
      assertKnownKeys(raw, ALL_KEYS3, TOOL4);
      const latexRaw = raw.latex;
      if (typeof latexRaw !== "string" || latexRaw.trim().length === 0) {
        throw new Error(`${TOOL4}: "latex" must be a non-empty string`);
      }
      if (latexRaw.length > 2e4) throw new Error(`${TOOL4}: "latex" is longer than 20000 characters`);
      const latex = stripMathDelimiters(latexRaw);
      if (latex.length === 0) throw new Error(`${TOOL4}: "latex" is empty after removing math delimiters`);
      const displayRaw = raw.display;
      if (displayRaw !== void 0 && typeof displayRaw !== "boolean") {
        throw new Error(`${TOOL4}: "display" must be a boolean`);
      }
      ...
      if (fontSizeRaw !== void 0 && (typeof fontSizeRaw !== "number" || !Number.isFinite(fontSizeRaw))) {
        throw new Error(`${TOOL4}: "font_size" must be a finite number`);
      }
      const fontSize = fontSizeRaw ?? config.fontSize;
      if (fontSize < 6 || fontSize > 96) throw new Error(`${TOOL4}: "font_size" must be between 6 and 96`);
      const colorRaw = raw.color;
      if (colorRaw !== void 0 && (typeof colorRaw !== "string" || !isSafeColor(colorRaw))) {
        throw new Error(`${TOOL4}: "color" must be a flat CSS color such as "#111827"`);
      }
      const color = colorRaw ?? config.color;
      const args = parseCommonArgs(raw, config, TOOL4);
      const root = resolveWorkspaceRoot(config, exec);
      const formula = await renderFormulaSvg(latex, {
        display,
        fontSize,
